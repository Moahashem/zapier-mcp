...
import { filterAndRename, type ToolMapping } from "./filter.js";
...
export async function buildServer(config: Config): Promise<BuiltServer> {
  ...
  log.info(
    `discovered ${initialTools.length} upstream tools → ` +
  ...
  if (initial.kept.length === 0) {
    log.warn(
      "no tools exposed. Open https://mcp.zapier.com, pick your server, and " +
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    try {
      ...
      log.debug(
        `list_tools: ${result.kept.length} exposed (${result.droppedCount} dropped)`,
      );
...
function indexMappings(kept: ToolMapping[]): Map<string, ToolMapping> {
  const m = new Map<string, ToolMapping>();
  for (const k of kept) m.set(k.exposedName, k);
  return m;
}
