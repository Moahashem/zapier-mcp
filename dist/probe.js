...
async function main() {
    ...
    // eslint-disable-next-line no-console
    console.log(`Match terms: ${config.matchTerms.length > 0 ? config.matchTerms.join(", ") : "(none — pass-through)"}`);
    // eslint-disable-next-line no-console
    ...
    // eslint-disable-next-line no-console
    console.log(`Of those, ${result.kept.length} would be exposed (${result.droppedCount} dropped).\n`);
